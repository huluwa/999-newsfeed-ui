...
import oms.sns.facebook.service.ObjectHandler;
import oms.sns.facebook.ui.view.ImageRun;
...
public class AsyncFacebook 
{
	...
	{
		getThreadPool().dispatch(new AsyncTask(PHONE_BOOK, listener, new Object[]{uid}) 
        {
         	public void invoke(FacebookListener listener,Object[] args) throws FacebookException {
                listener.getPhoneBooks(getPhoneBooks((Long)args[0]));
            }
        });
	}	 
	public void getPhoneBooksInBackAsync(long uid, int limit, int offset,  FacebookListener listener ) 
    {
        getBackThreadPool().dispatch(new AsyncTask(PHONE_BOOK, listener, new Object[]{uid, limit, offset}) 
        {
            public void invoke(FacebookListener listener,Object[] args) throws FacebookException {
                listener.getPhoneBooks(getPhoneBooks((Long)args[0], (Integer)args[1], (Integer)args[2]));
            }
        });
    }    
  ...
  public void batch_run_getPageInfoAndUserInfo(long[] uids,FacebookListener listener) {
    ...
    public static QueuedThreadPool getBackThreadPool()
    {
        synchronized(QueuedThreadPool.class)
        {
            if(null == threadbackpool){
                threadbackpool = new QueuedThreadPool(3);
    ...
    private static final int NetworkPoolSize=15;   
