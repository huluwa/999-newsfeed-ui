...
import com.tormas.litesina.ui.adapter.RecipientsAdapter;
import com.tormas.litesina.ui.adapter.SimplyCommentsAdapter;
...
import com.tormas.litesina.AddressPad;
import com.tormas.litesina.AddressPad.AddressDecorator;
import com.tormas.litesina.AddressPad.OnAddressKeyListener;
...
public class TwitterComposeActivity extends StatusViewBaseActivity {
	...
	long   status_id;
 	...
 	private Button    twitter_mail_send_button ;
 	private Button    twitter_mail_reply_cancel;
 	private View       receiver_span;
 	private ListView  commentsView;
 	private List<String> currentSelected = new ArrayList<String>();
 	private ArrayList<SimplyComments>status = new ArrayList<SimplyComments>();
    ...
    public void onCreate(Bundle savedInstanceState) 
	{
        ...
        twitter_mail_send_button.setOnClickListener(new View.OnClickListener() {			
			public void onClick(View arg0) {
				titleSelected();
			}
		});
        
        twitter_mail_reply_cancel.setOnClickListener(new View.OnClickListener() {			
			public void onClick(View arg0) {
        ...
        Log.d(TAG, "twitter_id="+to_uid +" status_id="+status_id + " reply="+isReply + " isRetweet="+isRetweet + " isDirect="+isDirect + " isUpdate"+isUpdate + " isComments="+isComments);
	...
	private void loadComments(final long statusid)
    {
		if(isInProcess())
		{
			Log.d(TAG, "in loading status comments="+statusid);
			return ;
		}
        ...
        twitterA.getSimplyCommentsAsync(String.valueOf(statusid), new TwitterAdapter() 
        {
			@Override public void gotSimplyComments(List<SimplyComments> statuses)
            {
				synchronized(mLock)
		    	{
		    	    inprocess = false;
                ...
                if(donotcallnetwork == false)//I am still alive
                {
	            	...
	            	if(statuses.size() > 0)
	            	{
    	            	...
    	            	handler.obtainMessage(GET_COMMENTS_UI).sendToTarget();
	            	}
                }  
                
                handler.obtainMessage(GET_COMMENTS_END).sendToTarget();
            }
	...
	private class HandlerLoad extends Handler 
    {
        ...
        public void handleMessage(Message msg) 
        {
            ...
            {  
	            case GET_COMMENTS:
	            {
	            	long statusid = msg.getData().getLong(STATUS_ID, -1);
	            	loadComments(statusid);
	            	break;
	            }
	            case GET_COMMENTS_UI:
	            {	
	            	SimplyCommentsAdapter ta = new SimplyCommentsAdapter(TwitterComposeActivity.this, status);
	                commentsView.setAdapter(ta);	                 
	            	break;
	            }
	...
	public class FBUDecorater implements AddressDecorator {
		public CharSequence decorateAddress(AddressPad addressPad,
			...
			String suid = address.toString().trim();
			 List<Follow> follows = orm.getTwitterUserByID(suid);                    
	            if(follows.size()>0){
