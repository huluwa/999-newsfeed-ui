...
import oms.sns.main.*;
import oms.sns.main.providers.SocialORM.Account;
import oms.sns.main.providers.SocialORM.Follow;
import oms.sns.main.ui.adapter.FollowAdapter;
import oms.sns.main.ui.adapter.SimplyStatusAdapter;
import oms.sns.main.ui.view.ImageRun;
...
public class TwitterUserDetailsActivity extends StatusViewBaseActivity {
	...
	private boolean forsearchfromtwitterlist = false;
	...
	private void processIntent()
	{
        ...
        isForOutSeach = this.getIntent().getBooleanExtra("search", false);       
        forsearchfromtwitterlist = this.getIntent().getBooleanExtra("forsearchfromtwitterlist", false);
        
        if(user != null)
        {
	        setTitle(R.string.twitter_user_detail_from_following);
	        
	        if(forsearchfromtwitterlist)
	        {
	        	getTwitterUserByScreenname(user.screenName);
	        }
        }
        //set ui
	...
	private void getUserDetailInfo(){
		twitterA.getUserDetailSimplyAsync(String.format("%1$s", user.screenName), new TwitterAdapter() 
	    {
	...
	public void titleSelected()
	{
	    ...
	    if(isForOutSeach == false)
	    {
        	...
        	intent.putExtra(TWITTER_ID, user.screenName);        	
	...
	{	
    	if(isForOutSeach)
    		...
    		finalTitle = getString(R.string.twitter_user_detail_from_following);
	...
	View.OnClickListener stopFollowingClick = new View.OnClickListener()
	{
		public void onClick(View v) 
		{
			if(isMyFriends)
			{
		        ...
		        msg.getData().putString(TWITTER_ID, user.screenName);		         
  ...
  //handler follow message
	...
	public void getUserLine() 
	{
	    ...
	    if(lastGetTime != -1)
	    {
		    ...
		    twitterA.getSimplyUserTimelineAsync(user.screenName,from , new TwitterAdapter() 
 	        {
		...
		{
		    twitterA.getSimplyUserTimelineAsync(user.screenName, new TwitterAdapter() 
 	        {
 		 	    @Override public void gotSimplyUserTimeline(List<SimplyStatus> statuses)
 	            {
	 			    lastGetTime = System.currentTimeMillis()+1000; 
