package com.msocial.facebook.widget.provider;

import java.util.ArrayList;
import java.util.List;

import com.msocial.facebook.widget.model.LiteStream;
...
public class WidgetORM {
	...
	public ArrayList<LiteStream> getStreams() {
		if (DEBUG)
			Log.d(TAG, "getStreams");
		ArrayList<LiteStream> list = new ArrayList<LiteStream>();
		...
		try {
			...
			if (cursor != null)
				while (cursor.moveToNext()) {
					LiteStream stream = new LiteStream();
	...
	public LiteStream getStreamsByPostId(String postid) {
		if (DEBUG)
			Log.d(TAG, "getStreamsByPostId" + postid);
		LiteStream stream = null;
		Uri uri = Uri.parse(WidgetProvider.CONTENT_URI + "/stream");
		String[] projection = { "_id", "post_id", "source_id", "message",
				"updated_time", "created_time" };
		Cursor cursor = null;
	...
	public boolean insertStream(LiteStream stream) {
		if (DEBUG)
	...
	public boolean addStream(LiteStream stream, int streams_num) {
		LiteStream dbstream = getStreamsByPostId(stream.post_id);
		if (dbstream != null) {
	...
	public void addStreams(List<LiteStream> streams, int streams_num) {
		for (LiteStream stream : streams)
