...
public class IconCache {
    ...
    public void flush() {
        synchronized (mCache) {
        	if(Launcher.LOGD)Log.d(TAG, "flush"+"  size:"+mCache.size());
            mCache.clear();
            isAllAppsDirty = true;
        }
    }
    
    private boolean isAllAppsDirty = true;
    public void setIsAllAppsCacheDirty(boolean dirty){
    	synchronized (mCache) {
    		isAllAppsDirty = dirty;
    	}
    }
    
    public boolean getIsAllAppsCacheDirty(){
    	return isAllAppsDirty;
    }
    ...
    private CacheEntry cacheLocked(ComponentName componentName, ResolveInfo info) {
        ...
        if (entry == null) {
     	...
     	//	if(Launcher.LOGD)Log.d(TAG, "cacheLocked componentName:"+componentName+"  size:"+mCache.size());
        }
    ...
    public void dump(){
    	Set<ComponentName> comKey = mCache.keySet();
    	Iterator<ComponentName> cns = comKey.iterator();
    	while(cns.hasNext()){
    		ComponentName cn = cns.next();
    		CacheEntry ce = (CacheEntry)mCache.get(cn);
    		Log.d("Launcher.iconcache",  "cn:"+cn+"  icon:"+ce.icon+"  titleBitmap:"+ce.titleBitmap);
    	}
    }
