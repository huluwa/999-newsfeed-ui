...
public class Category {
	...
	public static void initialRes(Context con){
		...
		if(nameAll == null){
			...
			drawableShortcut       = res.getDrawable(R.drawable.cmcc_launcher_setting_quick_start);
...
//		}
	 ...
	 * default return true;
	 */
	public static boolean addApps(Context con, ArrayList<ApplicationInfo> list, int currentCategory){
		...
		//need check whether in current category, if installed new app, and it also in category, need show it immediately
		addApplicationCategory(con, list);
...
//				+" carrierapps:"+carrierapps.size()+" oemapps:"+oemapps.size()+" the3rdapps:"+the3rdapps.size());
	...
	private static void addApplicationCategory(Context context, ArrayList<ApplicationInfo>newapps){
		if(Launcher.LOGD)Log.d(TAG, "addApplicationCategory="+newapps.size());
		...
		if(enintents != null && enintents.size()>0){
			...
			for(int i=0; i<N; i++){				
				String str = enintents.get(i).Intent;
				for(int j=0; j<newapps.size(); j++){
					ApplicationInfo info = newapps.get(j);
	...
	public static void addApplicationCategory(Context con, int cid, ApplicationInfo app){
		addApplicationCategory(con, cid, app, false);
	}
	
	public static void addApplicationCategory(Context con, int cid, ApplicationInfo app, boolean notoast){
		if(cid == CATEGORY_Entertainment)
		{
			for(int i=0; i<entertainmentapps.size(); i++){
				final ApplicationInfo info = entertainmentapps.get(i);
				if(notoast==false && info.intent.toUri(0).equals(app.intent.toUri(0)))
				{
					Toast.makeText(con, String.format(con.getString(R.string.app_exist), info.title), Toast.LENGTH_SHORT).show();
					return;
				}
			}
