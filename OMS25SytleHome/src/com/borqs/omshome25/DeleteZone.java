...
public class DeleteZone extends ImageView implements DropTarget, DragController.DragListener {
    ...
    private static   boolean mRealTrashMode;    
    ...
    private Animation mbottomLayoutInAnimation;
    private Animation mbottomLayoutOutAnimation;
    ...
    public static synchronized boolean isInTrashMode()
    {
    	return mRealTrashMode == true;
    }
...
//    }
    ...
    public void onDrop(DragSource source, int x, int y, int xOffset, int yOffset,
            DragView dragView, Object dragInfo) {
        if(Launcher.LOGD)Log.d(TAG,"onDrop");
        ...
        mLauncher.mDesktopItems.remove(item);
    }

    public void onDragEnter(DragSource source, int x, int y, int xOffset, int yOffset,
            DragView dragView, Object dragInfo) {
    	if(mTransition == null){
    		setImageResource(R.drawable.delete_zone_selector);
    		mTransition = (TransitionDrawable) getDrawable();
    	}
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
        ...
        if (item != null) {
            ...
            if(mLauncher.dockStyle == 0){
            	mbottomLayout.startAnimation(mbottomLayoutOutAnimation);
            }
    ...
    public void onDragEnd() {
        if (mTrashMode) {
            ...
            if(mLauncher.dockStyle == 0){
            	mbottomLayout.startAnimation(mbottomLayoutInAnimation);
            }
            setVisibility(GONE);
        }
    }
    ...
    private void createAnimations() {
        ...
        if(mLauncher.dockStyle == 0){
        	if (mbottomLayoutInAnimation == null) {
        		if(mOrientation == ORIENTATION_HORIZONTAL){
        			mbottomLayoutInAnimation = new TranslateAnimation(Animation.ABSOLUTE, 0.0f,
        					Animation.ABSOLUTE, 0.0f, Animation.RELATIVE_TO_SELF, 1.0f,
        					Animation.RELATIVE_TO_SELF, 0.0f);
        		}else{
        			mbottomLayoutInAnimation = new TranslateAnimation(Animation.RELATIVE_TO_SELF,
        					1.0f, Animation.RELATIVE_TO_SELF, 0.0f, Animation.ABSOLUTE, 0.0f,
        					Animation.ABSOLUTE, 0.0f);
        		}
        		
        		mbottomLayoutInAnimation.setDuration(ANIMATION_DURATION);
        	}
        	
        	if (mbottomLayoutOutAnimation == null) {
        		if(mOrientation == ORIENTATION_HORIZONTAL){
        			mbottomLayoutOutAnimation = new FastTranslateAnimation(Animation.ABSOLUTE, 0.0f,
        					Animation.ABSOLUTE, 0.0f, Animation.RELATIVE_TO_SELF, 0.0f,
        					Animation.RELATIVE_TO_SELF, 1.0f);
        		}else{
        			mbottomLayoutOutAnimation = new FastTranslateAnimation(Animation.RELATIVE_TO_SELF,
        					0.0f, Animation.RELATIVE_TO_SELF, 1.0f, Animation.ABSOLUTE, 0.0f,
        					Animation.ABSOLUTE, 0.0f);
        		}
        		
        		mbottomLayoutOutAnimation.setFillAfter(true);
        		mbottomLayoutOutAnimation.setDuration(ANIMATION_DURATION);
        	}
        }
    }
    ...
    void setBottom(RelativeLayout bottomLayout) {
