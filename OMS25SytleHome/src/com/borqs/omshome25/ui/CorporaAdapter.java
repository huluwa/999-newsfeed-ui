...
import com.borqs.omshome25.Category;
...
public class CorporaAdapter extends BaseAdapter {
    ...
    private Corpus allCorpus, systemCorpus, carrierappCorpus,
                   oemCorpus, the3rdCorpus, entertainmentCorpus, 
                   informationCorpus, toolsCorpus, shortcutCorpus, gameCorpus;
...
//    	if(Category.carrierapps.size()>0 && (SystemProperties.getInt("home_category_support_carrier", 1) == 1)){
        if((SystemProperties.getInt("home_category_support_carrier", 0) == 1)){
		...
		if((SystemProperties.getInt("home_category_support_shortcut", 0) == 1)){
		...
		if(supportMoreCustomize)
		{
			...
			if((SystemProperties.getInt("home_category_support_game", 1) == 1)){
				gameCorpus = new Corpus();
				gameCorpus.label = Category.nameGame;
				gameCorpus.icon  = Category.drawableGame;
				gameCorpus.category = Category.CATEGORY_GAME;
		    	mRankedEnabledCorpora.add(gameCorpus);
			}
