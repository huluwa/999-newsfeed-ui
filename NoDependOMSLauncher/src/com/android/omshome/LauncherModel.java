...
//    private Handler mRsClearHandler = new Handler();
    ...
    public interface Callbacks {
        ...
        public boolean isFinishedBindItems(int itemCount);
...
//        if(Launcher.LOGD)Log.d(TAG,"initialize");
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(intent.getAction().equals("omshome.backup_screennumber")){
    		...
    		in.putExtra("com.android.omshome.screen_num",     screen_count);
    		in.putExtra("com.android.omshome.default_screen", default_screen);
    		context.sendBroadcast(in);
    		return;
    	}
        else if(intent.getAction().equals("omshome.restore_screennumber")){     	
        	int number     = intent.getIntExtra("com.android.omshome.screen_num", Workspace.MAX_SCREEN_COUNT);
        	int def_number = intent.getIntExtra("com.android.omshome.default_screen", Workspace.DEFAULT_CURRENT_SCREEN);
...
//                                        }
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
                                ...
                                if (appWidgetId < 1000000 && !isSafeMode && 
                                    ...
                                    if (!checkItemPlacement(occupied, appWidgetInfo)) {
                                        ...
                                        Log.e(TAG, "already have appwidget " + id + ", removing it");
                                        contentResolver.delete(LauncherSettings.Favorites.getContentUri(
                                                     id, false), null, null);
            ...
            private void bindWorkspace() {
                ...
                N = mItems.size();
                {
                	final Callbacks bindcallbacks = tryGetCallbacks(oldCallbacks);
                    mHandler.setCallback(bindcallbacks, N);
                }
                for (int i=0; i<N; i+=ITEMS_CHUNK) {
                    final int start = i;
                    final int chunkSize = (i+ITEMS_CHUNK <= N) ? ITEMS_CHUNK : (N-i);
                    mHandler.postBind(new Runnable() {
                        public void run() {                        	
...
//                });
                ...
                mHandler.postFinishBind(new Runnable() {
                    public void run() {
                        Callbacks callbacks = tryGetCallbacks(oldCallbacks);
                        if (callbacks != null) {
