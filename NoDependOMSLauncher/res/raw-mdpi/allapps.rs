...
void updateReadback() {
    //debugF("=======b191 updateReadback g_OldPosPage", g_OldPosPage);
   // debugF("=======b191 updateReadback PosPage", g_PosPage);
...
void init() {
    //debugF("b191===entering init",g_PosPage);
...
void moveTo() {
    ...
    g_MoveToOldPos = g_PosPage;
	 ...
	 //debugF("=======b191 moveTo", state->targetPos);
}

void setZoom() {
    //debugF("=======b191 setZoom g_PosPage", g_PosPage);
    //debugF("=======b191 setZoom g_PosPage", g_PosPage);
...
void updatePos() {
   //debugF("b191===entering updatePos==",state->targetPos);
   //debugF("b191===entering updatePos==",g_PosPage);
    if (g_LastTouchDown) {
        return;
    }
...
draw_home_button()
{
    ...
    if (getWidth() > getHeight()) {
        ...
        y -= 10; // move the house to the edge of the screen as it doesn't fill the texture.
    }
...
void drawFrontGrid(float rowOffset, float p)
{
    ...
    float colWidth = w/4;//110.f; //120.f; 
    float rowHeight = colWidth /*+ 10.f*/;
    ...
    for (row = -5; row < 15; row++) {
        float y = yoff - ((-rowFrac + row) * rowHeight)+0.1f*rowHeight;
        ...
        for (col=0; col < colCount; col++) {
            ...
            if (iconNum >= 0) {
                ...
                vpConstants->ImgSize.x = 80.f;//120.f;
                vpConstants->ImgSize.y = 35.f;//64.f;
                vpConstants->Position.y = y - 35.f - 0.2f;//y - 64.f - 0.2f;
...
void draw_Loading_Animation()
{
    ...
    setColor(1.0f, 1.0f, 1.0f, 1.0f);
    bindTexture(NAMED_PFTexNearest, 0, NAMED_TLoading);
    ...
    matrixLoadRotate(mat1, -rotate, 0, 0, 1);
    rotate +=6;
    ...
    float y;
        ...
        y = (h - ICON_TEXTURE_HEIGHT_PX) * 0.5f; 
        x = (w - ICON_TEXTURE_HEIGHT_PX) / 2;
      ...
      vpConstants->ImgSize.x = ICON_TEXTURE_WIDTH_PX;
      vpConstants->ImgSize.y = ICON_TEXTURE_HEIGHT_PX;
      vpConstants->Position.y = y + ICON_TEXTURE_HEIGHT_PX/2+5;
            vpConstants->Position.x = x + ICON_TEXTURE_HEIGHT_PX/2 ;          
      drawSimpleMesh(NAMED_SMCell);        
...
main(int launchID)
{
   // debugF("b191==entering main",g_Zoom);
   // debugF("b191==entering main",state->zoomTarget);
    ...
    if ((g_Zoom < 0.001f) && (state->zoomTarget < 0.001f) && !g_SpecialHWWar) {
        ...
        if (!g_LastTouchDown) {
            //debugF("b191==entering main reset g_PosPage=0",g_PosPage);
            //g_PosPage = 0;
        }
        return lastFrame(0);
    } else {
    ...
    //gm_PosPage = g_PosPage;
    //draw_page_indicator(); //for page indicator
    ...
    return lastFrame((g_PosVelocity != 0) || fracf(g_PosPage) || g_Zoom != state->zoomTarget || (g_MoveToTime != 0||state->iconCount == 0));
}
