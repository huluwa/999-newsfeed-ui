...
draw_home_button()
{
    ...
    if (getWidth() > getHeight()) {
        ...
        y -= 20; // move the house to the edge of the screen as it doesn't fill the texture.
    }
...
void drawFrontGrid(float rowOffset, float p)
{
    ...
    for (row = -5; row < 15; row++) {
        float y = yoff - ((-rowFrac + row) * rowHeight) - 0.2f*rowHeight;
        //debugF("b1911 row=",row);
        //debugF("b1911y=",y);
        //debugF("b1911 yoff=",yoff);
...
void draw_Loading_Animation()
{
    ...
    float y;
      ...
      vpConstants->ImgSize.x = 200;
      vpConstants->ImgSize.y = 200;
      vpConstants->Position.y = y -125;
