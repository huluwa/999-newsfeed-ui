            ...
            if(uidd !=null && uidd.trim().length() > 0)
            {
            	...
            	try
            	{
                    ...
                    if(tempval != oldval)
                    {
                        Intent i = new Intent();
                        i.setClassName("com.ast.free", "com.ast.free.service.SNSService");
                        i.setAction("com.ast.free.intent.action.reset.FACEBOOK_PHONEBOOK");
                        this.startService(i);
                    }
		...
		else if(key.equals("key_facebook_phonebook_sync_period"))
		{
            ...
            if(uidd !=null && uidd.trim().length() > 0)
            {
            	...
            	try
            	{
                    ...
                    Intent i = new Intent();
                    i.setClassName("com.ast.free", "com.ast.free.service.SNSService");
                    i.setAction("com.ast.free.intent.action.reset.FACEBOOK_PHONEBOOK_SYNC");
                    this.startService(i);
		...
		else if(key.equals("key_facebook_mail_update_period"))
		{
            ...
            if(uidd !=null && uidd.trim().length() > 0)
            {
            	...
            	try{
                     ...
                     Intent i = new Intent();
                     i.setClassName("com.ast.free", "com.ast.free.service.SNSService");
                     i.setAction("com.ast.free.intent.action.reset.MAIL_CHECK");
                     this.startService(i);
            	}catch(NumberFormatException ne){
            	    Toast.makeText(this.getApplicationContext(),R.string.sns_setting_numeric_checking, Toast.LENGTH_SHORT).show();
            	}           
