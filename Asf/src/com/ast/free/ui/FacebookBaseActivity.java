...
import com.ast.free.ui.ActivityBase.TitleListener;
import com.ast.free.ui.FacebookAlbumActivity.FacebookAlbumItemView;
import com.ast.free.ui.FacebookAlbumViewActivity.AlbumItemView;
import com.ast.free.ui.view.CommentItemView;
import com.ast.free.ui.FacebookNotificationManActivity.NotesItemView;
import com.ast.free.ui.view.FacebookEventItemView;
import com.ast.free.ui.view.FacebookFriendItemView;
import com.ast.free.ui.view.FacebookMailItemView;
import com.ast.free.ui.view.FacebookNotificationItemView;
import com.ast.free.ui.view.FacebookStatusView;
import com.ast.free.ui.view.MessageItemView;
import com.ast.free.ui.view.MessageThreadInfoItemView;
import com.ast.free.ui.view.SNSItemView;

import com.ast.free.util.StatusNotification;
...
public abstract class FacebookBaseActivity extends ActivityBase implements
    ...
    protected static final String ACTION_CHECK_CONTECT = "com.ast.free.intent.action.ACTION_CHECK_CONTECT";
   ...
   GestureDetector mGestureDetector;
    	...
    	basichandler.postDelayed(new Runnable()
    	{
    		public void run()
    		{
    			long now = System.currentTimeMillis();
    	    	 long pre = now;
    	         String value = orm.getSettingValue("trail_version");
    	         if(isEmpty(value))
    	         {
    	         	orm.addSetting("trail_version", String.valueOf(now));
    	         }
    	         else
    	         {
    	        	 try{
    	         	    pre = Long.parseLong(value);
    	        	 }catch(Exception ne)
    	        	 {
    	        		 orm.addSetting("trail_version", String.valueOf(now));
    	        	 }
    	     	...
    	     	if((now-pre) > 15*24*3600*1000L)
    	     	{
    	     		Toast.makeText(FacebookBaseActivity.this, "Trial version expired, thanks", 5000).show();
    	     		basichandler.postDelayed( new Runnable()
    	     		{
    	     			public void run()
    	     			{
    	     	...
    	     	{
    	     		Toast.makeText(FacebookBaseActivity.this, "You still have "+ (15 - (int)((now-pre)/(24*3600*1000L))) + " days to use this software.", 5000).show();
    	     	}
    ...
    public static final String TARGET_DEVICE="penguinlite".trim().toLowerCase();
    public static final String TARGET_DEVICE2="penguin".trim().toLowerCase();
    
    public static final String TARGET_DEVICE_Blu="Blu".trim().toLowerCase();
    public static final String TARGET_DEVICE_vibo_A300="Vibo-A300".trim().toLowerCase(); 
    public static final String TARGET_DEVICE_vibo_A600="Vibo-A600".trim().toLowerCase();

    public static TextView getLinkfiedView(Context ctx, int msg) {

          final TextView message = new TextView(ctx);

          message.setTextColor(Color.WHITE);
          final SpannableString s = new SpannableString(ctx.getText(msg));
        ...
        if(true)
        {
	        ...
	        else
	        {
	        	new AlertDialog.Builder(this).setView(
	                    getLinkfiedView(this, R.string.not_target_device)).setCancelable(
	                    true).setNegativeButton("OK", null).show();
	        }
        }
    }
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		int useback = SystemProperties.getInt("user_facebook_background", 0);
		if(true || useback != 0)
		{
    ...
    */
    ...
    {        
        ...
        Log.i(TAG, "after super.onDestroy()" + this);
    }
    ...
    public void onCreateContextMenu(ContextMenu menu, View v,
        ...
        {
            contextMenu.findItem(R.id.facebook_menu_reply).setVisible(false);
            if(orm.isMailEnableVersion())contextMenu.findItem(R.id.facebook_menu_forward).setVisible(true);
        ...
        {
            if(orm.isWallEnaboeVersion())contextMenu.findItem(R.id.facebook_menu_wall_post).setVisible(true);
        ...
        {
            if(orm.isWallEnaboeVersion())contextMenu.findItem(R.id.facebook_menu_wall_post).setVisible(true);
            if(orm.isMailEnableVersion())contextMenu.findItem(R.id.facebook_con_menu_new_message).setVisible(true);
        ...
        {
            if (AlbumItemView.class.isInstance(i.targetView) && orm.isPhotoEnableVersion()) {
        ...
        else if (FacebookAccountActivity.class.isInstance(this)) 
        {
            if (FacebookAlbumItemView.class.isInstance(i.targetView)) {
                ...
                if (fa.getPhotoAlbum().owner == perm_session.getLogerInUserID() && orm.isPhotoEnableVersion()) {
                    contextMenu.findItem(R.id.facebook_menu_photo_comments).setVisible(true);
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (loginHelper.getPermanentSesstion(this) != null) {
            ...
            if(orm.isFullVersion())menu.setGroupVisible(R.id.facebook_group_about_nemu, true);
            if(orm.isFullVersion())menu.findItem(R.id.facebook_menu_about).setVisible(true);
        ...
        else if (FacebookProfileActivity.class.isInstance(this)) 
        {
            ...
            if(orm.isUnLimitVersion())menu.findItem(R.id.facebook_menu_go_mobile_facebook).setVisible(true);
            menu.setGroupVisible(R.id.facebook_group_about_nemu, true);
            menu.findItem(R.id.facebook_menu_settings).setVisible(true);
            if(orm.isFullVersion())menu.findItem(R.id.facebook_menu_status_update).setVisible(true);
        } 
        else if (FacebookAccountActivity.class.isInstance(this)) 
        {
            if(orm.isMailEnableVersion())menu.findItem(R.id.facebook_menu_new_mail).setVisible(true);
            
            /*
            if(getCurrentTab() != 0)menu.findItem(R.id.facebook_menu_profile_wall).setVisible(true);
            if(getCurrentTab() != 1)menu.findItem(R.id.facebook_menu_profile_info).setVisible(true);
            if(getCurrentTab() != 2)menu.findItem(R.id.facebook_menu_profile_photo).setVisible(true);
            */
            ...
            if (((FacebookAccountActivity) (this)).comefrommyself) 
            {
                ...
                menu.findItem(R.id.facebook_menu_ext_permission).setVisible(false);
                menu.findItem(R.id.facebook_menu_friends).setVisible(false);
                if(orm.isFullVersion())menu.findItem(R.id.facebook_menu_find_friends).setVisible(true);
                menu.findItem(R.id.facebook_menu_phonebook).setVisible(false);
                if(orm.isFullVersion())menu.findItem(R.id.facebook_menu_set_contact).setVisible(true);
                menu.findItem(R.id.facebook_menu_friend_group).setTitle(R.string.facebook_groups_title);
                if(orm.isFullVersion())menu.findItem(R.id.facebook_menu_friend_group).setVisible(true);
        ...
        {
            menu.findItem(R.id.facebook_menu_refresh).setVisible(true);
            if(orm.isFullVersion())menu.findItem(R.id.facebook_menu_friend_birthday).setVisible(true);
            //menu.findItem(R.id.facebook_menu_add_event).setVisible(true);
            menu.findItem(R.id.facebook_menu_clear_cache).setVisible(true);
            //menu.findItem(R.id.facebook_menu_sync_event).setVisible(true);
            if(orm.isFullVersion())menu.findItem(R.id.facebook_menu_event_switch).setVisible(true);
        ...
        else if (FacebookAlbumViewActivity.class.isInstance(this)) 
        {
            ...
            if (act.isOwner() && orm.isPhotoEnableVersion()) {
        ...
        {
        	menu.findItem(R.id.facebook_menu_refresh).setVisible(true);
        	if(orm.isFullVersion())menu.findItem(R.id.facebook_menu_phonebook).setVisible(true);
        	if(orm.isFullVersion())menu.findItem(R.id.facebook_menu_sync_addressbook).setVisible(true);        	
        }
    ...
    protected void loadProfile(int type)
    {
    ...
    public boolean onOptionsItemSelected(MenuItem item) 
    {
        switch (item.getItemId()) 
        {
	        ...
	        case R.id.facebook_menu_profile_wall:
	        {
	        	loadProfile(0);
	        	break;
	        }
	        case R.id.facebook_menu_profile_info:
	        {
	        	loadProfile(1);
	        	break;
	        }
	        case R.id.facebook_menu_profile_photo:
	        {
	        	loadProfile(2);
	        	break;
	        }	       
