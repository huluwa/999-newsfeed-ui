...
{	
    ...
    public void stopLoading()
    {
        ...
        synchronized(connections)
        {
            ...
            while(it.hasNext())
            {
            	...
            	{
            		((oms.sns.service.facebook.client.AsyncFacebook.AsyncTask) run).setStoped(true);
            	}
	...
	{	
	    ...
	    {
	        Log.d(TAG, "why come here= ProcessNoExtPermission");
	    }
	    else
	    {
    		Log.d(TAG, "no ext permission, need uer comfirm perm="+permission);
    		/*
        	Message msg = basichandler.obtainMessage(NO_EXT_PERMISSION);
        	msg.getData().putString("permission", permission);
        	msg.sendToTarget();
        	*/
    		
    		//prompt a dialog to let user select OK or cancel
    		if(ActivityBase.this.isFinishing() == false)
    		{
    			SocialORM orm = SocialORM.instance(ActivityBase.this);
    			if(orm.isEnableAssignPermission() == true)
    			{
		    		basichandler.post( new Runnable()
		    		{
		    			public void run()
		    			{
		    				Intent diaglogIntent = new Intent(SNSService.getSNSService(),FacebookPermssionDialog.class);
		             	    diaglogIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);	             	    
		             	    FacebookPermssionDialog.appendMessage(permission);	
		             	    SNSService.getSNSService().getApplicationContext().startActivity(diaglogIntent);
		    			}
		    		});
    			}
    			else
    			{
    				basichandler.post( new Runnable()
    	    		{
    	    			public void run()
    	    			{
        			        Toast.makeText(ActivityBase.this, String.format(ActivityBase.this.getString(R.string.hint_no_permission), permission), Toast.LENGTH_LONG).show();
    	    			}
    	    		});
    			}
    		}
    		...
    		{
    			Log.d(TAG, "I am finishing, so no UI again");
    		}
	    }
	}
	...
	public static boolean isInAynscTaskAndStoped()
	{
         ...
         if(PoolThread.class.isInstance(th))
         {
         	...
         	if(run != null && oms.sns.service.facebook.client.AsyncFacebook.AsyncTask.class.isInstance(run))
        	{
        		isStoped = ((oms.sns.service.facebook.client.AsyncFacebook.AsyncTask) run).Stoped();
        	}
         }
