...
public class SNSReceiver extends BroadcastReceiver 
{
    ...
    public void onReceive(Context context, Intent intent) 
    {
        ...
        else if(intent.getAction().equals(android.content.Intent.ACTION_BOOT_COMPLETED)) 
        {
            ...
            if(perm_session == null)
            {
                ...
                Log.d(TAG, "after kill com.msocial.nofree process="+pid);
            }
            else
            {
        ...
        {            
        	...
        	File files = new File("/data/data/com.msocial.nofree/files/");
        	if(files.exists())
        	{
        		//try 
        		//{
					//Runtime.getRuntime().exec("rm /data/data/com.msocial.nofree/files/* ");
	...
	private class ClearCacheThread extends Thread
	{
        @Override
        public void run() {
            File file = new File("/data/data/com.msocial.nofree/files/");
            deleteFile(file);
            clearDatabase();
    ...
    private void clearDatabase() {
        //check database 
        // if sns.db    
        File file = new File("/data/data/com.msocial.nofree/databases/sns.db");
