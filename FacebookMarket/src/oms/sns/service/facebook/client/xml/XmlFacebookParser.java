...
public class XmlFacebookParser {
		...
		for(org.w3c.dom.Node cnode=node.getFirstChild();cnode != null;cnode=cnode.getNextSibling())
		{
			...
			{					
				...
				if(name.equalsIgnoreCase("current_location"))
			    {
			    	...
			    	for(org.w3c.dom.Node mnode=cl.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
		            {
	            		 if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
	                     {                   
	                         String cname = mnode.getNodeName();
	                         String val = getChildText(cname, (Element)mnode);    
	                         if(val.equals("")) continue;
	                         else if(cname.equalsIgnoreCase("city")) facebookuser.current_location.city = val;
	                         else if(cname.equalsIgnoreCase("country")) facebookuser.current_location.country = val;                         
	                         else if(cname.equalsIgnoreCase("state")) facebookuser.current_location.state = val;
	                         else if(cname.equalsIgnoreCase("zip")) facebookuser.current_location.zip = val;	                         
	                     }
		            }
			    }
				else if (name.equalsIgnoreCase("hometown_location"))
				{
			    	...
			    	for(org.w3c.dom.Node mnode=cl.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
		            {
	                     ...
	                     {                   
	                         ...
	                         String val = getChildText(cname, (Element)mnode);    
	                         if(val.equals("")) continue;
	                         else if(cname.equalsIgnoreCase("city")) facebookuser.hometown_location.city = val;
	                         else if(cname.equalsIgnoreCase("country")) facebookuser.hometown_location.country = val;                         
	                         else if(cname.equalsIgnoreCase("state")) facebookuser.hometown_location.state = val;
	                         else if(cname.equalsIgnoreCase("zip")) facebookuser.hometown_location.zip = val;	                         
	                     }
		            }
				}
								...
								{	
									...
									for(org.w3c.dom.Node like_node=edu_node.getFirstChild();like_node != null;like_node=like_node.getNextSibling())
									{
										...
										{	
											...
											{
												String val_s = getChildText("name", (Element)like_node);																								
												object.name = val_s;
											}
											else if("year".equals(nodename))
											{
												String val_s = getChildText("year", (Element)like_node);																								
												object.year = val_s;
											}
											else if("degree".equals(nodename))
											{
												String val_s = getChildText("degree", (Element)like_node);																								
												object.degree = val_s;
											}
											else if("school_type".equals(nodename))
											{
												String val_s = getChildText("school_type", (Element)like_node);
						...
						for(org.w3c.dom.Node work_node=works.getFirstChild();work_node != null;work_node=work_node.getNextSibling())
						{
							...
							{	
								...
								{	
									...
									for(org.w3c.dom.Node mnode=edu_node.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
						            {
					            		 if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
					                     {                   
					                         String cname = mnode.getNodeName();
					                         String val = getChildText(cname, (Element)mnode);    
					                         if(val.equals("")) continue;
					                         else if(cname.equalsIgnoreCase("company_name")) object.company_name = val;
					                         else if(cname.equalsIgnoreCase("position")) object.position = val;                         
					                         else if(cname.equalsIgnoreCase("description")) object.description = val;
					                         else if(cname.equalsIgnoreCase("start_date")) object.start_date = val;
					                         else if(cname.equalsIgnoreCase("end_date")) object.end_date = val;	  
					                     }
						            }
									...
									if(cons != null && cons.getLength() > 0)
									{
										...
										{
											for(org.w3c.dom.Node mnode=con_node.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
								            {
							            		 if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
							                     {                   
							                         String cname = mnode.getNodeName();
							                         String val = getChildText(cname, (Element)mnode);    
							                         if(val.equals("")) continue;
							                         else if(cname.equalsIgnoreCase("city")) object.city = val;
							                         else if(cname.equalsIgnoreCase("state")) object.state = val;                         
							                         else if(cname.equalsIgnoreCase("country")) object.country = val;							                          
							                     }
								            }
										}
									}										
                ...
                {   
                    ...
                    for(org.w3c.dom.Node mnode=s_node.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
		            {
	            		 if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
	                     {                   
	                         String cname = mnode.getNodeName();
	                         String val = getChildText(cname, (Element)mnode);    
	                         if(val.equals("")) continue;
	                         else if(cname.equalsIgnoreCase("message")) facebookuser.message = val;
	                         else if(cname.equalsIgnoreCase("status_id")) facebookuser.statusid = Long.parseLong(val);                         
	                         else if(cname.equalsIgnoreCase("time")) facebookuser.statustime = Long.parseLong(val);							                          
	                     }
		            }
                } 
		        else//root value
	...
	public static String getChildText(String str, Element elem) 
	{
        ...
        String localName = elem.getNodeName();
   	    ...
   	    NodeList nl = null;
        if(isEmpty(localName) == false && localName.equalsIgnoreCase(str))
        {
        	 nl = elem.getChildNodes();
        }
        else
        {
        	nl = elem.getElementsByTagName(str);
        }
   ...
   public static List<MailboxMessage> parseMailMessage(Element elem){
				...
				{	
					if(name.equals("likes"))
					{
						...
						if(likes.getChildNodes() != null && likes.getChildNodes().getLength() >0)
						{
						    ...
						    for(org.w3c.dom.Node mnode=likes.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
				            {
			            		 if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
			                     {                   
			                         String cname = mnode.getNodeName();
			                         String val = getChildText(cname, (Element)mnode);    
			                         if(val.equals("")) continue;
			                         else if(cname.equalsIgnoreCase("can_like")) stream.likes.can_like = Integer.valueOf(val);
			                         else if(cname.equalsIgnoreCase("count"))  stream.likes.count = Integer.valueOf(val);                        
			                         else if(cname.equalsIgnoreCase("user_likes")) stream.likes.user_likes = Integer.valueOf(val);							                          
			                     }
				            }
					...
					else if(name.equals("comments"))
					{
						...
						if(comments.getChildNodes() != null && comments.getChildNodes().getLength() >0)
						{
							...
							for(org.w3c.dom.Node com_node=comments.getFirstChild();com_node != null;com_node=com_node.getNextSibling())
							{
								...
								if(com_node.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
								{	
									if("count".equals(com_name))
									{
										 String val = getChildText(com_name, (Element)com_node);
										 try{
												stream.comments.count = Integer.valueOf(val);
										 }catch(Exception ne){}
									}
									else if("comment_list".equals(com_name))
									{	
										...
										{	
											...
											{	
												if("comment".equals(post_name))
												{
										            ...
										            for(org.w3c.dom.Node ccnode=m_node.getFirstChild();ccnode != null;ccnode=ccnode.getNextSibling())
										            {
										                if(ccnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
										                {                   
										                    String cname = ccnode.getNodeName();
										                    String val = getChildText(cname, (Element)ccnode);    
										                    if(val.equals("")) continue;
										                    else if(cname.equalsIgnoreCase("fromid")) comment_item.fromid = (Long.valueOf(val));
										                    else if(cname.equalsIgnoreCase("time")) comment_item.time = Long.valueOf(val)*1000;
										                    else if(cname.equalsIgnoreCase("text")) comment_item.text = (val);                   
										                    else if(cname.equalsIgnoreCase("username")) comment_item.username = val;
										                    else if(cname.equalsIgnoreCase("id")) comment_item.id = val;
					...
					else if(name.equals("attachment"))
					{
						...
						if(att_elem.getChildNodes() != null && att_elem.getChildNodes().getLength()> 0)
						{
							...
							for(org.w3c.dom.Node mnode=att_elem.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
				            {
			            		 if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
			                     {                   
			                         String cname = mnode.getNodeName();
			                         String val = getChildText(cname, (Element)mnode);    
			                         if(val.equals("")) continue;
			                         else if(cname.equalsIgnoreCase("name")) stream.attachment.name = val;
			                         else if(cname.equalsIgnoreCase("href")) stream.attachment.href = val;                        
			                         else if(cname.equalsIgnoreCase("caption")) stream.attachment.caption = val;
			                         else if(cname.equalsIgnoreCase("description")) stream.attachment.description = val;	
			                         else if(cname.equalsIgnoreCase("icon")) stream.attachment.icon = val;	
			                     }
				            }
										...
										{	
											...
											{	
												if("stream_media".equals(com_name))
												{
													...
													for(org.w3c.dom.Node mnode=m_node.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
										            {
									            		 if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
									                     {                   
									                         String cname = mnode.getNodeName();
									                         String val = getChildText(cname, (Element)mnode);    
									                         if(val.equals("")) continue;
									                         else if(cname.equalsIgnoreCase("type")) media_item.type = val;									                         
									                         else if(cname.equalsIgnoreCase("src")) media_item.src = val;	
									                         else if(cname.equalsIgnoreCase("href")) media_item.href = val;	
									                     }
										            }
	...
	public static List<AppInfo> parseAppinfo(Document doc) {
	    ...
	    for(int i=0;i<appsnode.getLength();i++)
        {
            ...
            if(appinfonode.getNodeType() == Element.ELEMENT_NODE)
            {
                ...
                for(org.w3c.dom.Node mnode=appinfoelement.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
	            {
            		 if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
                     {                   
                         String cname = mnode.getNodeName();
                         String val = getChildText(cname, (Element)mnode);    
                         if(val.equals("")) continue;
                         else if(cname.equalsIgnoreCase("app_id")) appinfo.app_id = Long.parseLong(val);
                         else if(cname.equalsIgnoreCase("api_key")) appinfo.api_key = val;									                         
                         else if(cname.equalsIgnoreCase("canvas_name")) appinfo.canvas_name = val;                         
                         else if(cname.equalsIgnoreCase("display_name")) appinfo.display_name = val;	
                         else if(cname.equalsIgnoreCase("icon_url")) appinfo.icon_url = val;	
                         else if(cname.equalsIgnoreCase("logo_url")) appinfo.logo_url = val;	
                         
                         else if(cname.equalsIgnoreCase("company_name")) appinfo.company_name = val;	
                         else if(cname.equalsIgnoreCase("description")) appinfo.description = val;	
                         else if(cname.equalsIgnoreCase("daily_active_users")) appinfo.daily_active_users = Long.parseLong(val);	
                         else if(cname.equalsIgnoreCase("weekly_active_users")) appinfo.weekly_active_users = Long.parseLong(val);	
                         else if(cname.equalsIgnoreCase("monthly_active_users")) appinfo.monthly_active_users = Long.parseLong(val);
                     }
	            }
                ...
                if(developers_nodelist!=null && developers_nodelist.getLength()>0)
                {
                    ...
                    for(int j=0;j<developers_nodelist.getLength();j++)
                    {
                        ...
                        for(org.w3c.dom.Node mnode=developerinfoelement.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
         	            {
                     		if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
                            {                   
                                String cname = mnode.getNodeName();
                                String val = getChildText(cname, (Element)mnode);    
                                if(val.equals("")) continue;
                                else if(cname.equalsIgnoreCase("uid")) developerinfo.uid = Long.parseLong(val);
                                else if(cname.equalsIgnoreCase("name")) developerinfo.name = val;									                         
                            }
         	            }
			...
			for(org.w3c.dom.Node cnode=stream_post.getFirstChild();cnode != null;cnode=cnode.getNextSibling())
			{
				...
				{	
					if(name.equals("messages"))
					{
						...
						{
							for(org.w3c.dom.Node mnode=messages.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
				            {
				                if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
				                {                   
				                    String mname = mnode.getNodeName();
				                    String val = getChildText(mname, (Element)mnode);    
				                    if(val.equals("")) continue;
				                    else if(mname.equalsIgnoreCase("unread")) notify.msg.unread = Integer.valueOf(val);
				                    else if(mname.equalsIgnoreCase("most_recent")) notify.msg.most_recent = Long.valueOf(val);				                   
				                }
				            }
					...
					else if(name.equals("pokes"))
					{
						...
						{
							for(org.w3c.dom.Node mnode=pokes.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
				            {
				                if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
				                {                   
				                    String mname = mnode.getNodeName();
				                    String val = getChildText(mname, (Element)mnode);    
				                    if(val.equals("")) continue;
				                    else if(mname.equalsIgnoreCase("unread")) notify.poke.unread = Integer.valueOf(val);
				                    else if(mname.equalsIgnoreCase("most_recent")) notify.poke.most_recent = Long.valueOf(val);				                   
				                }
				            }
					...
					else if(name.equals("shares"))
					{
						...
						{
							for(org.w3c.dom.Node mnode=shares.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
				            {
								if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
				                {                   
				                    String mname = mnode.getNodeName();
				                    String val = getChildText(mname, (Element)mnode);    
				                    if(val.equals("")) continue;
				                    else if(mname.equalsIgnoreCase("unread")) notify.share.unread = Integer.valueOf(val);
				                    else if(mname.equalsIgnoreCase("most_recent")) notify.share.most_recent = Long.valueOf(val);				                   
				                }
				            }
						}						
					}
	...
	static void parseNotification(NodeList notificationsnode,Notifications notifications)
	{
	    ...
	    for(int i=0;i<notificationsnode.getLength();i++)
        {
            ...
            if(notificationnode.getNodeType() == Element.ELEMENT_NODE)
            {
                ...
                for(org.w3c.dom.Node mnode=notificationelement.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
	            {
					if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
	                {                   
	                    String mname = mnode.getNodeName();
	                    String val = getChildText(mname, (Element)mnode);    
	                    if(val.equals("")) continue;
	                    else if(mname.equalsIgnoreCase("is_hidden")) notification.is_hidden  =(!isEmpty(val)&&val.equals("1")?true:false);
	                    else if(mname.equalsIgnoreCase("is_unread")) notification.is_unread  = (!isEmpty(val)&&val.equals("1")?true:false);				                   
	                    else if(mname.equalsIgnoreCase("app_id")) notification.app_id  =Long.parseLong(val);
	                    else if(mname.equalsIgnoreCase("href")) notification.href  = val;
	                    else if(mname.equalsIgnoreCase("body_text")) notification.body_text  = val;
	                    else if(mname.equalsIgnoreCase("body_html")) notification.body_html  = val;
	                    else if(mname.equalsIgnoreCase("title_text")) notification.title_text= val;
	                    else if(mname.equalsIgnoreCase("title_html")) notification.title_html  = val;
	                    
	                    else if(mname.equalsIgnoreCase("updated_time")) 
	                    {
	                    	if(isEmpty(val) == false)
	                        	try{
	                            notification.updated_time  = Long.parseLong(val)*1000L;
	                        	}catch(Exception ne){}
	                        else
	                            notification.updated_time = notification.created_time;
	                    }
	                    else if(mname.equalsIgnoreCase("created_time")) notification.created_time= Long.parseLong(val)*1000L; 
	                    else if(mname.equalsIgnoreCase("recipient_id")) notification.recipient_id  = Long.parseLong(val);
	                    else if(mname.equalsIgnoreCase("sender_id")) notification.sender_id  = Long.parseLong(val);
	                    else if(mname.equalsIgnoreCase("notification_id")) notification.notification_id  = Long.parseLong(val);
	                }
	            }
	...
	public static List<PhotoComment> parserPhotoComments(Document doc,FacebookSession session)
	{
        ...
        for(int i=0;i<size;i++)
        {
            ...
            if(comment_node.getChildNodes() != null && comment_node.getChildNodes().getLength() >0)
            {
            	...
            	for(org.w3c.dom.Node mnode=comment_node.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
	            {
            		 if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
                     {                   
                         String name = mnode.getNodeName();
                         String val = getChildText(name, (Element)mnode);    
                         if(val.equals("")) continue;
                         else if(name.equalsIgnoreCase("pid")) photoComment.pid = val;
                         else if(name.equalsIgnoreCase("time")) photoComment.time = Long.valueOf(val)*1000;                         
                         else if(name.equalsIgnoreCase("from")) photoComment.from = Long.valueOf(val);
                         else if(name.equalsIgnoreCase("body")) photoComment.body = val;
                         else if(name.equalsIgnoreCase("pcid")) photoComment.pcid = val;
                     }
	            }
	...
	public static Comments parserStreamComments(Document doc,long source_id,FacebookSession session)
	{
        ...
        for(int i=0;i<size;i++)
        {
            ...
            Stream.Comments.Stream_Post comment_item= new Stream_Post();       
            for(org.w3c.dom.Node cnode=comment_node.getFirstChild();cnode != null;cnode=cnode.getNextSibling())
            {
                if(cnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
                {                   
                    String name = cnode.getNodeName();
                    String val = getChildText(name, (Element)cnode);    
                    if(val.equals("")) continue;
                    else if(name.equalsIgnoreCase("fromid")) comment_item.fromid = (Long.valueOf(val));
                    else if(name.equalsIgnoreCase("time")) comment_item.time = Long.valueOf(val)*1000;
                    else if(name.equalsIgnoreCase("text")) comment_item.text = (val);                   
                    else if(name.equalsIgnoreCase("username")) comment_item.username = val;
                    else if(name.equalsIgnoreCase("id")) comment_item.id = val;
                }
            }    
					...
					if(att_elem.getChildNodes() != null && att_elem.getChildNodes().getLength()> 0)
					{
						...
						for(org.w3c.dom.Node mnode=att_elem.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
	    	            {
	                        ...
	                        {                   
	                            ...
	                            String val = getChildText(cname, (Element)mnode);    
	                            if(val.equals("")) continue;
	                            else if(cname.equalsIgnoreCase("note_id")) note.note_id = Long.parseLong(val);
	                            else if(cname.equalsIgnoreCase("title")) note.title = val;
	                            else if(cname.equalsIgnoreCase("content")) note.content = val;	
	                            else if(cname.equalsIgnoreCase("created_time")) note.created_time = Long.parseLong(val)*1000L;	
	                            else if(cname.equalsIgnoreCase("updated_time")) note.updated_time = Long.parseLong(val)*1000L;	
	                            else if(cname.equalsIgnoreCase("uid")) note.uid = Long.parseLong(val);	
	                        }
	    	            }
			...
			for(int i=0;i<list.getLength();i++){
				...
				for(org.w3c.dom.Node cnode=node.getFirstChild();cnode != null;cnode=cnode.getNextSibling())
				{
					...
					{					
						...
						String val = getChildText(name, (Element)cnode);
						if(val.equals("")) continue;
						else if(name.equalsIgnoreCase(Page.Field.PAGE_ID.toString())) {try{ page.page_id = Long.valueOf(val);}catch(Exception ne){}}
						else if(name.equalsIgnoreCase(Page.Field.NAME.toString())) page.name = val;
						else if(name.equalsIgnoreCase(Page.Field.PAGE_URL.toString())) page.page_url = val;
						else if(name.equalsIgnoreCase(Page.Field.PIC_SMALL.toString())) page.pic_small = val;
						else if(name.equalsIgnoreCase(Page.Field.PIC_SQUARE.toString())) page.pic_square = val;	
	 ...
	 public static List<FriendRelationship> areMyFriends(Document doc){
		 ...
		 for(int i=0;i<nodelist.getLength();i++){
			 ...
			 if(oneNode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
			 {
				 ...
				 for(org.w3c.dom.Node mnode=oneNode.getFirstChild();mnode != null;mnode=mnode.getNextSibling())
 	             {
             		if(mnode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE)
                     {                   
                         String cname = mnode.getNodeName();
                         String val = getChildText(cname, (Element)mnode);    
                         if(val.equals("")) continue;
                         else if(cname.equalsIgnoreCase("uid1")) fr.uid1 = Long.parseLong(val);
                         else if(cname.equalsIgnoreCase("uid2")) fr.uid2 = Long.parseLong(val);
                         else if(cname.equalsIgnoreCase("are_friends")) fr.isFriends = val.trim().equals("1");                         
                     }
 	             }
				 fids.add(fr);
			 }					
